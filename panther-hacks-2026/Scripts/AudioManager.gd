...
# All your AudioStreamPlayer nodes go here as children of AudioManager.
# Just make sure the node names in your scene match the names below.
...
func _ready():
	...
	sounds["underwater_ambiance"].play()
	sounds["bubble_pop_collect"].play()
	sounds["boat_floating"].play()

	# Start both music tracks — danger starts silent
	sounds["MusicExplore"].play()
	sounds["enemy_nearby"].play()
	sounds["enemy_nearby"].volume_db = -80.0
...
func play(sound_name):
	if sound_name == "damage_player_grunt":
...
func danger_music(active):
	...
	if active:
		...
		sounds["enemy_detection"].play()
		tween.tween_property(sounds["enemy_nearby"],  "volume_db", 0.0,   1.5)
		tween.tween_property(sounds["enemy_nearby"], "volume_db", -80.0, 1.5)
	else:
...
func set_boat_proximity(t):
	sounds["boat_floating"].volume_db = lerp(-30.0, -8.0, t)
